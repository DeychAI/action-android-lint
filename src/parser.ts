...
export const parseXmls = async (files: string[], ignoreWarnings: boolean): Promise<Annotation[]> => {
...
export const parseXml = async (text: string, ignoreWarnings: boolean): Promise<Annotation[]> => {
  ...
  return new Promise(resolve => {
    try {
      ...
      for (const issueElement of xml.issues.issue) {
        ...
        if (ignoreWarnings && issue.severity == 'warning') {
          continue
        }
